...
/**
 * User-specialized ServiceStep:
 * - Syncs with Guest Booking aesthetics (1:1 with ServiceStep.jsx)
 * - Authenticated users are allowed to book specialized services by default
 */
const UserServiceStep = ({ selectedServiceId, onSelect, onNext }) => {
    ...
    const handleServiceSelect = (service) => {
        // Authenticated users can always select any service without restrictions
        ...
        <div className="pb-[60px] sm:pb-6">
            ...
            {loading && (
                <div className='grid grid-cols-2 lg:grid-cols-3 gap-3 sm:gap-6'>
                    {[1, 2, 3, 4, 5, 6].map(i => (
                        <div key={i} className='h-[160px] sm:h-[180px] rounded-2xl bg-gray-200 dark:bg-gray-700/60 animate-pulse border border-gray-100 dark:border-gray-800 shadow-theme-xs'></div>
                    ))}
                </div>
            )}

            {fetchError && (
                <div className='bg-red-50 dark:bg-red-900/10 border border-red-100 dark:border-red-900/20 text-red-600 dark:text-red-400 p-6 rounded-2xl text-center shadow-theme-sm'>
                        ...
                        >
                            ...
                            <div className='flex items-center justify-between mt-auto pt-3 sm:pt-4 border-t border-gray-100 dark:border-gray-800'>
                                <div className='flex items-center gap-2 sm:gap-3'>
                                    ...
                                    {service.tier?.toLowerCase() === 'specialized' && (
            ...
            {!loading && !fetchError && (!services || services.length === 0) && (
                <div className='text-center text-gray-400 py-20 bg-white/50 dark:bg-gray-800/20 rounded-3xl border border-dashed border-gray-200 dark:border-gray-800'>
